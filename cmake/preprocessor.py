...
class Preprocessor:
    ...
    def __init__(self, defines=None, marker='#'):
        ...
        context_items_iter = None
        try:
            context_items_iter = context_items.iteritems
        except AttributeError:
            context_items_iter = context_items.items
        for k,v in context_items_iter():
            self.context[k] = v
        self.actionLevel = 0
        self.disableLevel = 0
        # ifStates can be
        ...
        cmd_items_iter = None
        try:
            cmd_items_iter = cmd_items.iteritems
        except AttributeError:
            cmd_items_iter = cmd_items.items
        for cmd, level in cmd_items_iter():
    ...
    def write(self, aLine):
        ...
        if filteredLine != aLine:
            self.actionLevel = 2
        self.out.write(filteredLine.encode())
    ...
    def do_filter(self, args):
        ...
        filterNames = sorted(current.keys())
    ...
    def do_include(self, args, filters=True):
        ...
        unicode_check = None
        try:
            unicode_check = unicode
        except NameError:
            unicode_check = str
        isName = type(args) == str or type(args) == unicode_check
